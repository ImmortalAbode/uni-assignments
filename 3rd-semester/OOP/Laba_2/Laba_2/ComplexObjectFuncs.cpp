	...
	Line_Segment** ptr_linesegments_mass, int count_size_mass_linesegments, Ring** ptr_rings_mass, int count_size_mass_rings)
{
	...
	bool show_complex_object{ true }; //рисовать составной объект или удалить объект, освободив память
	bool move_complex_object{ false }; //позволить сотавному объекту двигаться или нет
	bool combine_objects{ false }; //есть ли работа с массивом объектов
	bool save_size_mass_complex_objects{ true };//сохранить размер массива для SHOW кнопки или нет
	bool create_30_complex_objects{ false }; //создать массив из 30 составных объектов, добавив его к основному
		...
		while (window.pollEvent(event_click))
		{
			...
			else if (event_click.type == event_click.MouseButtonReleased && event_click.mouseButton.button == sf::Mouse::Left)
			{
				...
				if (is_stage == 1)
				{
					if (create_30_complex_objects)
					{
						...
						if (count_size_mass_complex_objects - 30 != 0) 
							std::cout << "Массив составных объектов был создан конструктором со случайными параметрами." << std::endl;
						else 
							std::cout << "Массив составных объектов был создан конструктором со случайными параметрами. Первое кольцо - конструктором по умолчанию." << std::endl;
						std::cout << "На окне SFML находится " << count_size_mass_complex_objects << " составных объектов." << std::endl;
					}
					else if (count_size_mass_complex_objects == 0)
					{
						...
						show_complex_object = true;
						save_size_mass_complex_objects = true;
						std::cout << "Составной объект " << count_size_mass_complex_objects << " был создан конструктором по умолчанию." << std::endl;
					}
					else
					{
						create_random_complex_object(ptr_complex_objects_mass, count_size_mass_complex_objects);
						show_complex_object = true;
						save_size_mass_complex_objects = true;
						std::cout << "Составной объект " << count_size_mass_complex_objects << " был создан конструктором со случайными параметрами." << std::endl;
					}
				}
				if (is_stage == 2 && count_size_mass_complex_objects != 0)
				{
					...
					if (!combine_objects)
					{
						if (move_complex_object) std::cout << "Передвижение последней созданной фигуры включено." << std::endl;
						else std::cout << "Передвижение последней созданной фигуры выключено." << std::endl;
					}
					else
					{
						if (move_complex_object) std::cout << "Передвижение всех созданных фигур включено." << std::endl;
				...
				if (is_stage == 3 && (count_size_mass_complex_objects != 0 || count_size_mass_complex_objects == 0 && !show_complex_object))
				{
					...
					if (!show_complex_object)
					{
						if (!combine_objects)
						{
							std::cout << "Последний созданный составной объект было стерто." << std::endl;
							delete_element_shape<ComplexObject>(ptr_complex_objects_mass, count_size_mass_complex_objects);
							move_complex_object = false;
						}
						else
						{
							std::cout << "Все составные объекты были стерты." << std::endl;
				...
				if (is_stage == 6 && move_complex_object && count_size_mass_complex_objects != 0)
				{
					if (!combine_objects)
					{
						std::cout << "Последний созданный составной объект был перемещен на случайные координаты [-50; 50]." << std::endl;
					...
					else
					{
						...
						std::cout << "Все составные объекты были перемещены на случайные координаты[-50; 50]." << std::endl;
					}
				}
				if (is_stage == 7 && count_size_mass_complex_objects != 0)
				{
					std::cout << "Составной объект " << count_size_mass_complex_objects << " был удален." << std::endl;
					save_size_mass_complex_objects = true;
					delete_element_shape<ComplexObject>(ptr_complex_objects_mass, count_size_mass_complex_objects);
