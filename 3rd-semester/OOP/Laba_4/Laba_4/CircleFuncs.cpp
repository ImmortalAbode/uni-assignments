...
void create_random_circle(TCircle**& ptr_circles_mass, int& count_size_mass_circles)
{
	...
	ptr_obj_circle = new TCircle{ restrict_radius, restrict_x, restrict_y };
	ptr_obj_circle->setOutlineColor(getRandomNumber(0, 255), getRandomNumber(0, 255), getRandomNumber(0, 255));
	ptr_obj_circle->setFillColor(getRandomNumber(0, 255), getRandomNumber(0, 255), getRandomNumber(0, 255));
	ptr_obj_circle->setVisibility(getRandomNumber(50, 255));
	push_back_shape<TCircle>(ptr_circles_mass, count_size_mass_circles, ptr_obj_circle);
}
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event_click))
		{
			...
			else if (event_click.type == event_click.MouseButtonReleased && event_click.mouseButton.button == sf::Mouse::Left)
			{
				...
				{
					std::cout << "Все окружности были стерты." << std::endl;
					delete_elements_shape<TCircle>(ptr_circles_mass, count_size_mass_circles);
					combine_objects = false;
					move_circle = false;
					std::cout << "Режим группового изменения свойств объектов был выключен. Возможность передвижения объектов выключена." << std::endl;
				}
			...
			else if (event_click.type == sf::Event::KeyPressed && move_circle)
			{
				...
				if (!combine_objects)
				{
					...
					{
						ptr_circles_mass[count_size_mass_circles - 1]->ChangeRadius(1);
					}
				...
				else
				{
					...
					if (event_click.key.code == sf::Keyboard::E)
					{
						for (unsigned i{}; i < count_size_mass_circles;i++)
						{
							...
							{
								ptr_circles_mass[i]->ChangeRadius(1);
							}
						}
					}
