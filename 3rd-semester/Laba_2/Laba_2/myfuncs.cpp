...
int stage_1_choice_shape(sf::RenderWindow& window)
{
	...
	/*Кнопка Ring*/ 
	User_Text_Button ring_text_button{ "Comic_Sans_MS.TTF", "Ring", 30, 660, 350 };
	User_Rect_Button ring_rect_button{ 630, 340, 5, 250, 60 };

	/*Кнопка ComplexObject*/
	User_Text_Button complex_object_text_button{ "Comic_Sans_MS.TTF", "Complex object", 30, 640, 430 };
	User_Rect_Button complex_object_rect_button{ 630, 420, 5, 250, 60 };
	...
	/*Работа с массивом класса Ring*/
	int count_size_mass_rings{}; //кол-во элементов в массиве
	Ring** ptr_rings_mass = new Ring * [count_size_mass_rings]; //массив колец

	/*Работа с массивом класса ComplexObject*/
	int count_size_mass_complex_objects{}; //Кол-во элементов в массиве
	ComplexObject** ptr_complex_objects_mass = new ComplexObject * [count_size_mass_complex_objects]; //массив составных объектов
	...
	while (window.isOpen())
	{
		...
		ring_text_button.m_setFillColorDefault(); ring_rect_button.m_setOutlineColorDefault();
		complex_object_text_button.m_setFillColorDefault(); complex_object_rect_button.m_setOutlineColorDefault();
		...
		{
			ring_text_button.m_setFillColor(255, 127, 39);
			ring_rect_button.m_setOutlineColor(165, 15, 203);
			is_stage = 4;
		}

		if (sf::IntRect(625, 410, 260, 80).contains(sf::Mouse::getPosition(window)))
		{
			complex_object_text_button.m_setFillColor(255, 127, 39);
			complex_object_rect_button.m_setOutlineColor(165, 15, 203);
			is_stage = 5;
		}
			...
			else if (event_click.type == event_click.MouseButtonReleased && event_click.mouseButton.button == sf::Mouse::Left)
			{
				...
				if (is_stage == 1)
				{
					...
					work_with_circles(window, ptr_circles_mass, count_size_mass_circles, ptr_rectangles_mass, count_size_mass_rectangles,
						ptr_linesegments_mass, count_size_mass_linesegments, ptr_rings_mass, count_size_mass_rings, ptr_complex_objects_mass, count_size_mass_complex_objects);
				}
				if (is_stage == 2)
				{
				...
				if (is_stage == 3)
				{
					...
					work_with_linesegment(window, ptr_linesegments_mass, count_size_mass_linesegments, ptr_circles_mass, count_size_mass_circles,
						ptr_rectangles_mass, count_size_mass_rectangles, ptr_rings_mass, count_size_mass_rings, ptr_complex_objects_mass, count_size_mass_complex_objects);
				};
				if (is_stage == 4)
				{
					std::cout << "Вы выбрали класс объектов Ring." << std::endl;
					work_with_rings(window, ptr_rings_mass, count_size_mass_rings, ptr_circles_mass, count_size_mass_circles,
						ptr_rectangles_mass, count_size_mass_rectangles, ptr_linesegments_mass, count_size_mass_linesegments,
						ptr_complex_objects_mass, count_size_mass_complex_objects);
				}
				if (is_stage == 5)
				{
					std::cout << "Вы выбрали класс объектов Complex object." << std::endl;
					work_with_complex_objects(window, ptr_complex_objects_mass, count_size_mass_complex_objects, ptr_circles_mass, count_size_mass_circles,
						ptr_rectangles_mass, count_size_mass_rectangles, ptr_linesegments_mass, count_size_mass_linesegments, ptr_rings_mass, count_size_mass_rings);
				}
				if (is_stage == 6)
				{
					delete_mass_objects<Circle>(ptr_circles_mass, count_size_mass_circles);
		...
		ring_rect_button.Show(window); ring_text_button.Show(window);
		complex_object_rect_button.Show(window); complex_object_text_button.Show(window);
