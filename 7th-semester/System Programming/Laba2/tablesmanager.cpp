...
void TablesManager::LoadOneLineToSettingsTable(QTableWidget *settings_table, const QString &command_counter)
{
    if (settings_table == nullptr){
        qDebug() << "Pointer to settings_table was empty in TablesManager::LoadOneLineToSettingsTable.";
        return;
    }
    else
    {
        int last_row = settings_table->rowCount() - 1;
        settings_table->setItem(last_row, 0, new QTableWidgetItem(command_counter));
    }
}
...
void TablesManager::LoadDefaultAssemblerSourceCode(QTextEdit *source_text_TextEdit)
{
    ...
    else
    {
        ...
        //Вставляем пример.
        this->LoadOneLineToAssemblerSourceCode(source_text_TextEdit, {"Prog1", "START", "0", ""});
        ...
        this->LoadOneLineToAssemblerSourceCode(source_text_TextEdit, {"", "END", "10", ""});
    }
}
void TablesManager::LoadDefaultAssemblerSourceCode2(QTextEdit *source_text_TextEdit)
{
    if (source_text_TextEdit == nullptr)
    {
        qDebug() << "Pointer to source_text_TextEdit was empty in TablesManager::LoadDefaultAssemblerSourceCode.";
...
void TablesManager::LoadDefaultAssemblerSourceCode3(QTextEdit *source_text_TextEdit)
{
    if (source_text_TextEdit == nullptr)
    {
        qDebug() << "Pointer to source_text_TextEdit was empty in TablesManager::LoadDefaultAssemblerSourceCode.";
