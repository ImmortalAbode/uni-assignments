    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->choice_example_comboBox->addItem("Пример по умолчанию (без ошибок) - прямая адресация");
    ui->choice_example_comboBox->addItem("Пример по умолчанию (без ошибок) - относительная адресация");
    ui->choice_example_comboBox->addItem("Пример по умолчанию (без ошибок) - смешанная адресация");
...
void MainWindow::on_choice_example_comboBox_activated(int index)
{
    ...
    ui->table_settings_tableWidget->clear();
    ui->table_settings_tableWidget->setColumnCount(0);
    ui->table_settings_tableWidget->setRowCount(0);
    ...
    case 1://Только относительная адресация
        TManager->LoadDefaultAssemblerSourceCode2(ui->source_text_textEdit);
        TManager->LoadDefaultAssemplerOperationCode(ui->table_operation_codes_tableWidget);
        break;
    case 2://Смешанная адресация
        TManager->LoadDefaultAssemblerSourceCode3(ui->source_text_textEdit);
        TManager->LoadDefaultAssemplerOperationCode(ui->table_operation_codes_tableWidget);
        break;
    default:
        break;
...
void MainWindow::on_table_settings_tableWidget_cellChanged(int row, int column)
{
    TManager->UpdateViewOfTable(ui->table_settings_tableWidget, row, column);
}
...
void MainWindow::on_second_pass_pushButton_clicked()
{
    ...
    if(!this->pp.LoadBinaryCodeText(ui->object_module_header_tableWidget, ui->table_settings_tableWidget, ui->second_pass_errors_textBrowser, ui->binary_code_textBrowser, sup_table, symb_table))
    {
        //Очищаем элементы интерфейса.
        ui->object_module_header_tableWidget->clear();
        ui->object_module_header_tableWidget->setColumnCount(0);
    ...
    else
    {
        ...
        ui->table_settings_tableWidget->resizeColumnsToContents();
    }
    //Визуализация кнопок.
